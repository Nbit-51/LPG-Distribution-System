...
def create_agency(data: AgencyCreate) -> dict:
    agency_id = execute_query(
        "INSERT INTO agencies (agency_name, address, contact_number, email, region) VALUES (%s,%s,%s,%s,%s)",
...
def get_agency_by_id(agency_id: int) -> dict | None:
    rows = execute_query("SELECT * FROM agencies WHERE agency_id = %s", (agency_id,))
    return rows[0] if rows else None


def list_agencies(page=1, page_size=20, region=None, is_active=None) -> dict:
    conditions, params = [], []
    if region:
...
def deactivate_agency(agency_id: int) -> dict:
    ...
    if not existing:
        raise ValueError(f"Agency {agency_id} not found.")
    execute_query("UPDATE agencies SET is_active = FALSE WHERE agency_id = %s", (agency_id,), fetch=False)
    return get_agency_by_id(agency_id)


def get_agency_stock_summary(agency_id: int) -> dict | None:
    rows = execute_query("SELECT * FROM vw_agency_stock WHERE agency_id = %s", (agency_id,))
    return rows[0] if rows else None
