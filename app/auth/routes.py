...
@router.post("/login")
def admin_login(request: Request, form: OAuth2PasswordRequestForm = Depends()):
    ip = request.client.host if request.client else None
    admin = service.authenticate_admin(form.username, form.password, ip)
...
def consumer_login(request: Request, body: ConsumerLogin):
    # Note: body.phone can be phone number or email address
    ip = request.client.host if request.client else None
    consumer = service.authenticate_consumer(body.phone, body.password, ip)
    token = service.create_consumer_token(consumer["consumer_id"])
    return {"access_token": token, "token_type": "bearer",
            "consumer_id": consumer["consumer_id"], "full_name": consumer["full_name"],
            "phone": consumer["phone"], "consumer_type": consumer["consumer_type"],
...
def agent_login(request: Request, body: AgentLoginBody):
    from app.delivery_agents import service as agent_svc
    ip = request.client.host if request.client else None
    agent = agent_svc.authenticate_agent(body.phone, body.password, ip)
